...
func GetInterestRates(request *pb.GetInterestRatesRequest) (*pb.GetInterestRatesResponse, error) {
	// 获取 gRPC 客户端连接
	clientConn, err := pool.GetGrpcClient("water-level-service")
	if err != nil {
		return nil, fmt.Errorf("获取grpc客户端失败: %w", err)
	...
	resp, err := c.GetInterestRates(ctx, request)
	if err != nil {
		return nil, fmt.Errorf("获取实时汇率失败: %w", err)
	}
	fmt.Printf("获取实时汇率成功. Resp: %v\r\n", resp)
	return resp, nil
}
